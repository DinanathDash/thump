...
- Droplet id: `thump`. It is also `ThumpDroplet.id` in Swift and `id` in `droplet.json`; the three must agree or the loader refuses the bundle.
- Swift product: `Thump`, a dynamic library. The harness target is `ThumpHarness`.
...
1. Edit `Sources/Thump/`. The manifest is `droplet.json`.
2. `droppykit build` writes `.build/Thump.droplet`, universal, linked against the
...
5. Put the bundle into Droppy Playground and confirm it loaded. Copy
   `.build/Thump.droplet` to
   `~/Library/Application Support/Droppy Playground/Droplets/thump/Thump.droplet`,
...
- **The principal class does nothing.** `ThumpPrincipal` is `@objc`, is named in the
...
- **No `main.swift`.** The harness entry is `@main` in
  `Sources/ThumpHarness/ThumpHarness.swift`, and a file named `main.swift` cannot
  coexist with `@main`.
...
```
Package.swift                     product Thump (dynamic) and ThumpHarness
droplet.json                      the manifest; Info.plist is generated from it
Sources/Thump/              the droplet
Sources/ThumpHarness/       the @main harness entry; never main.swift
Thump.icon/                 Icon Composer document, required
Assets/Creator.png                square creator avatar, at least 256px, required
.build/Thump.droplet        what droppykit build writes
